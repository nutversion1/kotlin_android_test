...
class GetCurrentDateUseCase constructor(
    private var easyAppRepository: EasyAppRepository,
) : MediatorUseCase<Request<String>, GetCurrentDateResponse?>() {
    override fun execute(parameters: Request<String>) {

        result.addSource(easyAppRepository.getCurrentDate(parameters)) {
            when (it) {
                is Result.Success -> {
